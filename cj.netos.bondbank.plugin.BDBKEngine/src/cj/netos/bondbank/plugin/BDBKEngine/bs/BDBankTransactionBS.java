...
import cj.netos.bondbank.bs.IBDBankBalanceBS;
...
public class BDBankTransactionBS implements IBDBankTransactionBS, BigDecimalConstants {
	...
	@Override
	public void cashoutBill(String key, String balanceType, String cashoutor, String identity,
	...
	@Override
	public void investBill(String bank, String invester, BigDecimal amount, EInvesterType type,
		...
		if (bill.getType() == EInvesterType.customer) {
			...
			bill.setMerchantBondAmount(new BigDecimal(0));
			BigDecimal feeRate = feeRate(bdBankPropertiesBS, bank);
			bill.setFeeRate(feeRate);
			BigDecimal feeAmount = amount.multiply(feeRate).setScale(scale, roundingMode);
			bill.setFeeAmount(feeAmount);
			// 消费者投白单情况是委托债券银行代买债，收取一定比率费用，除此费用之外全部买债
			...
			BigDecimal customerBondAmount = amount.multiply(customerBondRate).setScale(scale, roundingMode);
			bill.setCustomerBondAmount(customerBondAmount);
			bill.setTotalBondAmount(new BigDecimal(0));
			bill.setMerchantDepositAmount(new BigDecimal(0));

			BigDecimal tailAmount = amount.subtract(customerBondAmount.add(feeAmount));// 不四约分
			bill.setTailAmount(tailAmount);

			buyBondAmount = customerBondAmount;
			...
			bill.setMerchantDepositAmount(merchantDepositAmount);
	...
	@Override
	public void exchangeBill(String key, String exchanger, BigDecimal bondQuantities,
			String informAddress) {
		// TODO Auto-generated method stub
	}

	@Override
	public void issueStockBill(String key, String issuer, BigDecimal bondQuantities,
			String informAddress) {
		// TODO Auto-generated method stub
	}
